...
class Level1_GameScene: SKScene, SKPhysicsContactDelegate  {
...
//    var locationsArray = [CGPoint]()
    ...
    func saveHighScore() {
       // UserDefaults.standard.set(self.gameScore, forKey: "HIGHSCORE")
        UserDefaults.standard.set(self.gameScore, forKey: "HIGHSCORE1")
        //self.highScore1 = UserDefaults().integer(forKey: "HIGHSCORE1")
    }
    ...
    override func didMove(to view: SKView) {
        print("sound on", UserDefaults().bool(forKey: "sound"))
        UserDefaults.standard.set(1, forKey: "currlevel")
        ...
        //TimerLabel.text = "Seconds Remaining: \(TimerNode)"
...
//        highscoreLabel.text = "HIGH SCORE: \(self.highScore1) "
...
//        highscoreLabel.zPosition = 100
        ...
        // Easy
        var tot_arr = [CGPoint]()
        var loc_arr = randomLocations(num: 8, tot_arr: tot_arr)
        tot_arr = loc_arr
        for i in 1...8{
            spawnCleanUp(location: loc_arr[i-1])
        }
        loc_arr = randomLocations(num: 4, tot_arr: tot_arr)
        for i in 1...4{
            spawnObstacle(location: loc_arr[i-1])
        }
        
        
        let background = SKSpriteNode(imageNamed: "livingroom1")
...
//        vroomba.physicsBody = SKPhysicsBody(circleOfRadius: 40)
        ...
        //vroomba.physicsBody!.categoryBitMask = vroombaCategory
        //vroomba.physicsBody!.collisionBitMask = None
       //vroomba.physicsBody!.contactTestBitMask = chargerCategory
        //vroomba.physicsBody!.contactTestBitMask = cleanUpCategory
        //vroomba.physicsBody!.contactTestBitMask = obstacleCategory
        //vroomba.physicsBody?.isDynamic = true
...
//            print(vroomba.position)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if body1.categoryBitMask == vroombaCategory && body2.categoryBitMask == obstacleCategory{
            //MusicPlayer.shared.playSoundEffect(soundEffect: "collision")
...
//              print("CHARGING...", health)
            ...
            if (clean == 0 && timer == true){
                ...
                scoreLabel.text = "SCORE:\(gameScore)"
                if self.gameScore > UserDefaults().integer(forKey: "HIGHSCORE1") {
                            saveHighScore()
                    print("New high score!!!")
                    highscoreLabel.text = "High Score: \(UserDefaults().integer(forKey: "HIGHSCORE1")) "
                        }
