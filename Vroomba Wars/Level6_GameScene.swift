...
class Level6_GameScene: SKScene, SKPhysicsContactDelegate  {
...
//    var locationsArray = [CGPoint]()
    ...
    func saveHighScore() {
       // UserDefaults.standard.set(self.gameScore, forKey: "HIGHSCORE")
        UserDefaults.standard.set(self.gameScore, forKey: "HIGHSCORE6")
    }
    ...
    func RandPointHard(tot_arr: [CGPoint]) -> CGPoint {
        let bad6 = CGPoint(x:4,y:10)
        let bad7 = CGPoint(x:0,y:5)
        let bad = CGPoint(x: 1, y: 0)
        let bad2 = CGPoint(x: 14, y: 4)
        let bad3 = CGPoint(x: 14, y: 0)
        let bad4 = CGPoint(x: 17, y: 8)
        let pos = CGPoint(x: random(min: 1, max: 19), y: random(min: 1, max: 10))
        if SquareCheck(p1: bad3, p2: bad4, point: pos){
            return RandPointHard(tot_arr: tot_arr)
        }
    ...
    func randomLocations(num: Int, tot_arr: [CGPoint])->[CGPoint]{
        ...
        for i in 1...num {
            let new_pos = RandPointHard(tot_arr: tot_arr1)
            loc_arr.append(new_pos)
            tot_arr1.append(new_pos)
            }
            
        for i in loc_arr.indices{
            loc_arr[i] = CGPoint(x: random(min:(ScreenSize.width/20) * loc_arr[i].x, max:(ScreenSize.width/20) * loc_arr[i].x+1), y: random(min:((ScreenSize.height*3/5)/10) * loc_arr[i].y, max:((ScreenSize.height*3/5)/10) * loc_arr[i].y+1))
        }
        return loc_arr
    }
    ...
    override func didMove(to view: SKView) {
        ...
        UserDefaults.standard.set(6, forKey: "currlevel")
...
//        highscoreLabel.text = "High Score: \(UserDefaults().integer(forKey: "HIGHSCORE5")) "
...
//        highscoreLabel.zPosition = 100
        ...
        var tot_arr = [CGPoint]()
        var loc_arr = randomLocations(num: 25, tot_arr: tot_arr)
        tot_arr = loc_arr
        for i in 1...25{
            spawnCleanUp(location: loc_arr[i-1])
        }
        loc_arr = randomLocations(num: 18, tot_arr: tot_arr)
        //17
        for i in 1...18{
        ...
        let background = SKSpriteNode(imageNamed: "kitchenBackground2")
...
//                UserDefaults.standard.set(true, forKey: "level6")
                UserDefaults.standard.set(true, forKey: "level7")
                self.gameScore += self.TimerNode
                scoreLabel.text = "SCORE:\(gameScore)"
                //self.TimerLabel.text = "Seconds Remaining: \(time)"
                if self.gameScore > UserDefaults().integer(forKey: "HIGHSCORE6") {
                            saveHighScore()
                    print("New high score!!!")
                    highscoreLabel.text = "High Score: \(UserDefaults().integer(forKey: "HIGHSCORE6")) "
                        }
            ...
            if (health == 0 && clean == 0 || timer == false && clean == 0){
                if self.gameScore > UserDefaults().integer(forKey: "HIGHSCOREE6") {
                            saveHighScore()
                    print("New high score!!!")
                    highscoreLabel.text = "High Score: \(UserDefaults().integer(forKey: "HIGHSCORE6")) "
                        }
