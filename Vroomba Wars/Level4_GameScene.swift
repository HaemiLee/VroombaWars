...
class Level4_GameScene: SKScene, SKPhysicsContactDelegate  {
...
//    var locationsArray = [CGPoint]()
    ...
    func saveHighScore() {
       // UserDefaults.standard.set(self.gameScore, forKey: "HIGHSCORE")
        UserDefaults.standard.set(self.gameScore, forKey: "HIGHSCORE4")
    }
    ...
    override func didMove(to view: SKView) {
        ...
        UserDefaults.standard.set(4, forKey: "currlevel")
...
//        highscoreLabel.text = "High Score: \(UserDefaults().integer(forKey: "HIGHSCORE4")) "
...
//        highscoreLabel.zPosition = 100
        ...
        var tot_arr = [CGPoint]()
        var loc_arr = randomLocations(num: 18, tot_arr: tot_arr)
        tot_arr = loc_arr
        for i in 1...18{
            spawnCleanUp(location: loc_arr[i-1])
        }
        loc_arr = randomLocations(num: 14, tot_arr: tot_arr)
        //14
        for i in 1...14{
        ...
        let background = SKSpriteNode(imageNamed: "bathroom")
...
//              print("CHARGING...", health)
            ...
            if (clean == 0 && timer == true){
                UserDefaults.standard.set(true, forKey: "level5")
                self.gameScore += self.TimerNode
                scoreLabel.text = "SCORE:\(gameScore)"
                //self.TimerLabel.text = "Seconds Remaining: \(time)"
                if self.gameScore > UserDefaults().integer(forKey: "HIGHSCORE4") {
                            saveHighScore()
